...
func (h *ProductHandler) CreateProduct(w http.ResponseWriter, r *http.Request) {
	if r.Header.Get("Content-Type") != "application/json" {
		...
		util.WriteError(w, http.StatusUnsupportedMediaType, "Content-Type must be application/json")
		return
	}
	...
	if err := h.v.Struct(req); err != nil {
		...
		util.WriteError(w, http.StatusBadRequest, "Validation failed")
		return
	}
	...
	util.WriteJSON(w, http.StatusCreated, p)
}
...
func (h *ProductHandler) GetProduct(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		util.WriteError(w, http.StatusBadRequest, "Invalid product ID")
		return
	}
...
func (h *ProductHandler) ListProducts(w http.ResponseWriter, r *http.Request) {
	...
	if limit < 0 || offset < 0 {
		...
		util.WriteError(w, http.StatusBadRequest, "Limit and offset must be non-negative")
		return
	}
...
func (h *ProductHandler) UpdateProduct(w http.ResponseWriter, r *http.Request) {
	...
	if req.Name == "" && req.Description == "" && req.Price == nil && req.Category == "" {
		...
		util.WriteError(w, http.StatusBadRequest, "At least one valid field must be provided")
		return
	}
